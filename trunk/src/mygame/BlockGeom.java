...
public class BlockGeom implements Callable {
    ...
    BlockGeom(int x_geom, int y_geom, int z_geom, int x_max, int y_max, int z_max, AssetManager assetmanager_)
    {
        ...
        for(x=0;x<LevelSize_max_x;x++)
            for(y=0;y<LevelSize_max_y/2;y++)
                for(z=0;z<LevelSize_max_z;z++)
                    blockarray[x][y][z] = (byte)(rand.nextInt(5) + 0);
         */
        
        //Level will be hardcoded here:        
        for(x=0;x<LevelSize_max_x;x++)
            for(y=0;y<190;y++)
                for(z=0;z<LevelSize_max_z;z++)
                    blockarray[x][y][z] = (byte)(2);
        
        for(x=0;x<LevelSize_max_x;x++)
            for(y=190;y<200;y++)
                for(z=0;z<LevelSize_max_z;z++)
                    blockarray[x][y][z] = (byte)(0);
        
        for(x=0;x<LevelSize_max_x;x++)
                for(z=0;z<LevelSize_max_z;z++)
                    blockarray[x][200][z] = (byte)(4);
        
        for(x=10;x<(LevelSize_max_x-10);x+=10)
                for(z=10;z<(LevelSize_max_z-10);z+=10)
                {
                    if((x==200) && (z==200))
                        z+=10;
                            
                    blockarray[x][201][z] = (byte)(3);
                    blockarray[x][202][z] = (byte)(3);

                    blockarray[x][203][z] = (byte)(4);
                    blockarray[x][204][z] = (byte)(4);
                    blockarray[x][205][z] = (byte)(4);

                    blockarray[x-1][203][z] = (byte)(4);
                    blockarray[x-1][204][z] = (byte)(4);
                    blockarray[x-1][205][z] = (byte)(4);

                    blockarray[x+1][203][z] = (byte)(4);
                    blockarray[x+1][204][z] = (byte)(4);
                    blockarray[x+1][205][z] = (byte)(4);

                    blockarray[x][203][z-1] = (byte)(4);
                    blockarray[x][204][z-1] = (byte)(4);
                    blockarray[x][205][z-1] = (byte)(4);
    ...
    private boolean DrawBlocks(int p_x, int p_y, int p_z, int radius)
    {
        int x,y,z;
        ...
        for(x=-radius;x<=radius;x++)
        {
            for(y=-radius;y<=radius;y++)
            {
                for(z=-radius;z<=radius;z++)
                {
                    ...
                    {
                        if(!EnableBlock(x+p_x,y+p_y,z+p_z))
                            return false;
                    }
                }
            }
    ...
    private boolean EnableBlock(int x, int y, int z)
    {
        //blockarray[x][y][z]=type;
